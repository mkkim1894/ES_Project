function makeFigure3_Generations(figureType, simParamsRef, varargin)
% makeFigure3_Generations — Generate Figure 3 (Modular FGM).
%
% Usage:
%   makeFigure3_Generations('ModularFGM', simParams)
%   makeFigure3_Generations('ModularFGM', simParams, 'test')
%   makeFigure3_Generations('ModularFGM', simParams, 'full', 'proximityCutoff', 0.1)
%
% Inputs:
%   figureType   - 'ModularFGM'
%   simParamsRef - simParams struct used to match files when multiple exist in the results directory
%
% Optional:
%   mode              - 'test', 'full', or 'auto' (positional)
%   'proximityCutoff' - Exclude data from ratio plots (D-F) when |x_i| < cutoff.
%                       Set to 0 to disable. Default: 0 (no cutoff).
%
% Outputs:
%   Saves Figure_ModularFGM_Generations.pdf to results/Figures/
...
fprintf('makeFigure3_Generations loading data from: %s\n', resultsRoot);
...
for i = 1:3
    ...
    if isscalar(files)
        ...
        if ~matched
            ...
            warning('Multiple files in %s. Using most recent: %s', dirPath, files(newest).name);
        end
    end
    fprintf('  Loading: %s\n', file_names{i});
end
