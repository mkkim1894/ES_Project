function makeFigure2(figureType, SSWMRate, CMRate, L_SSWM, L_CM)
    ...
    % L_SSWM: Genome length for the SSWM regime
    % L_CM: Genome length for the CM (asexual and sexual) regimes
    ...
    baseDirs = struct(...
        'SSWM', 'results/pleiotropicFGM/SSWM', ...
        'CM_Asexual', 'results/pleiotropicFGM/CM_Asexual', ...
        'CM_Sexual', 'results/pleiotropicFGM/CM_Sexual');
    ...
    for i = 1:3
        ...
        if strcmp(dirs{i}, 'SSWM')
            % SSWM uses its own mutation rate and genome length
            filePattern = sprintf('%s_%s_L%d_M%.1e.mat', figureType, dirs{i}, L_SSWM, SSWMRate);
        else
            % CM regimes use the CM mutation rate and genome length
            if strcmp(dirs{i}, 'CM_Sexual')
                filePattern = sprintf('%s_%s_L%d_M%.1e_R*.mat', figureType, dirs{i}, L_CM, CMRate);
            else
                filePattern = sprintf('%s_%s_L%d_M%.1e.mat', figureType, dirs{i}, L_CM, CMRate);
            end
        end
        files = dir(fullfile(dirPath, filePattern));
        if isempty(files)
            error('No files found in %s for %s with mutation rate %.1e and genome length L=%d', ...
                dirPath, figureType, strcmp(dirs{i}, 'SSWM') * SSWMRate + ~strcmp(dirs{i}, 'SSWM') * CMRate, ...
                strcmp(dirs{i}, 'SSWM') * L_SSWM + ~strcmp(dirs{i}, 'SSWM') * L_CM);
        end
    ...
    % Save the figure
    saveFigure('figure2', figureWidth, figureHeight);
end
